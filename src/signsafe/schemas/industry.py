...
# US-law presets deprecated in RU v1. Enum values are KEPT (removing them breaks
# existing clients), but a request for one returns an explicit "режим не поддерживается"
# result — never silently re-analyzed under a different (RU) legal regime.
DEPRECATED_INDUSTRIES: set[str] = {"hoa", "auto_purchase", "medical_bill", "elder_care"}
...
INDUSTRY_FOCUS: dict[str, str] = {
    ...
    "medical_bill": (
        ...
        "Оценивай каждую строку. Указывай ориентировочную сумму переплаты. "
        "Формулируй осторожно ('вероятно', 'стоит уточнить'), без юридических указаний."
    ),
    "employment": (
        ...
        "- НЕКОНКУРЕНТНАЯ ОГОВОРКА: запрет работать в отрасли после увольнения (в РФ, вероятно, не подлежит принудительному исполнению)\n"
        ...
        "Говори понятно для работника без юридического образования. "
        "Формулируй осторожно, без директив и ссылок на номера статей."
    ),
    "loan": (
        ...
        "- ШТРАФ ЗА ДОСРОЧНОЕ ПОГАШЕНИЕ: ограничение права досрочного погашения (вероятно, неправомерно)\n"
        ...
        "Формулируй осторожно ('вероятно', 'стоит уточнить'), без ссылок на номера статей."
    ),
    "purchase": (
    ...
    "service": (
        ...
        "Формулируй осторожно ('вероятно', 'можно попробовать оспорить'), без номеров статей."
    ),
    "insurance": (
    ...
    "residential_lease": (
        "ДОГОВОР НАЙМА ЖИЛЬЯ — ГЛАЗАМИ НАНИМАТЕЛЯ.\n"
        "Это договор найма жилого помещения. Читатель — наниматель, физлицо. "
        "Твоя задача — защитить его от невыгодных и незаконных условий.\n"
        "Для найма жилья физлицом используй термины «наниматель» и «наймодатель» "
        "(не «арендатор»/«арендодатель»).\n"
        "Обрати особое внимание на типичные ловушки для нанимателя и присвой "
        ...
        "- ШТРАФ ЗА ДОСРОЧНОЕ РАСТОРЖЕНИЕ нанимателем → clause_type=early_termination\n"
        "- ОДНОСТОРОННЕЕ ПОВЫШЕНИЕ ПЛАТЫ ЗА НАЁМ (чаще раза в год / по усмотрению "
        "наймодателя) → clause_type=rent_escalation\n"
        "- ОДНОСТОРОННЕЕ ИЗМЕНЕНИЕ ДРУГИХ УСЛОВИЙ наймодателем → clause_type=unilateral_change\n"
        "- ОТВЕТСТВЕННОСТЬ НАНИМАТЕЛЯ ЗА ЕСТЕСТВЕННЫЙ ИЗНОС → clause_type=maintenance_shift\n"
        "- ЗАПРЕТ НА РЕГИСТРАЦИЮ / ПРОЖИВАНИЕ ЧЛЕНОВ СЕМЬИ ИЛИ ТРЕТЬИХ ЛИЦ "
        "→ clause_type=third_party_restriction\n"
        "- ВНЕСУДЕБНОЕ ВЫСЕЛЕНИЕ / НЕМОТИВИРОВАННОЕ ДОСРОЧНОЕ РАСТОРЖЕНИЕ НАЙМОДАТЕЛЕМ "
        "→ clause_type=landlord_termination\n"
        "Говори понятным языком для нанимателя без юридического образования. "
        "НЕ ссылайся на конкретные номера статей — проверенные ссылки на нормы "
        "добавляются системой отдельно. "
        "НЕ давай директив ('подавайте в суд', 'не платите') — только осторожно "
        "информируй о возможных рисках ('вероятно', 'можно попробовать оспорить')."
    ),
    "auto_purchase": (
...
def is_deprecated_industry(industry: str | None) -> bool:
    return (industry or "").lower() in DEPRECATED_INDUSTRIES
