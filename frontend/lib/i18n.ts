...
export const UI_EN: Record<string, string> = {
  ...
  "consent.storage":
    "PDF обрабатывается в оперативной памяти и удаляется сразу после извлечения текста. Сервер не хранит ни файл, ни его текст, ни результат разбора — результат сохраняется только в вашем браузере.",
  "consent.authority":
  ...
  "privacy.s5.body":
    "На сервере — ничего. PDF живёт в оперативной памяти несколько секунд и удаляется после извлечения текста. Ни файл, ни текст, ни результат разбора, ни факт вашего согласия на сервере не сохраняются. Результат записывается только в localStorage вашего браузера — очистка данных сайта удаляет его безвозвратно.",
