...
export const UI_EN: Record<string, string> = {
  ...
  "consent.transfer":
    "Замаскированный текст договора передаётся для анализа зарубежному AI-провайдеру OpenRouter (за пределами России). Маскирование снижает, но не исключает риск передачи персональных данных.",
  "consent.storage":
  ...
  "privacy.s4.body":
    "Замаскированный текст договора передаётся AI-провайдеру OpenRouter и обрабатывается на серверах за пределами Российской Федерации. Это трансграничная передача. Мы используем бесплатные модели, поэтому не можем гарантировать, что провайдер не использует переданный текст для собственных целей. Не загружайте договоры, содержание которых для вас критично конфиденциально.",
